...
export default function StadiumCreatePage() {
  ...
  // This useEffect is no longer needed as colors are managed by the backend
  // useEffect(() => {
  ...
  async function handleCreateStadium(e: React.FormEvent) {
    e.preventDefault();
    setError('');
    
    try {
      // Create school profile with AI
      const schoolProfile = await createSchoolProfile(schoolName, {
      ...
      const identity = await buildIdentity({
        name: mascot, // Using mascot as username for now
        school: schoolName,
        sport: sports[0], // Primary sport
        position: sports.length > 0 ? 'Multi-Sport Athlete' : undefined,
        graduationYear: new Date().getFullYear() + 2
      });

      // Generate stadium design if not already generated
      let finalDesign = ''; // Placeholder for previewImage
      if (!finalDesign) {
        const design = await generateDesign({
          type: 'stadium_background',
          context: {
            school: schoolName,
            colors: [colors.primary, colors.secondary], // Assuming colors are managed by backend
            style: theme
          }
        });
      ...
      const enhancedData = {
        ...
        theme,
        bannerStyle,
        highlights: [], // Placeholder, will be generated by backend
      ...
      if (response.ok) {
        ...
        window.location.href = `/stadium/${mascot}`; // Assuming mascot is the username
      } else {
        const data = await response.json();
        setError(data.error || 'Failed to create stadium.');
      }
    } catch (error) {
            ...
            {step === 1 && (
              ...
              >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={mascot} // Changed to mascot
                  ...
                  />
                  <p className="text-white/50 text-sm mt-1">This will be your stadium URL: ultrapreps.com/stadium/{mascot || 'your-username'}</p>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        <input
                          type="color"
                          value={colors.primary} // Placeholder for colors
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          type="color"
                          value={colors.secondary} // Placeholder for colors
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      <button
                        key={value}
                        onClick={() => setTheme(value as StadiumData['theme'])}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      <button
                        key={value}
                        onClick={() => setBannerStyle(value as StadiumData['bannerStyle'])}
                ...
                <div className="grid grid-cols-2 gap-3 max-h-96 overflow-y-auto">
                  {activityOptions.map((activity) => (
                    <button
                      key={activity}
                      onClick={() => {
                        const newActivities = sports.includes(activity)
                          ? sports.filter(s => s !== activity)
                          : [...sports, activity];
                        setSports(newActivities);
                      }}
                      className={`p-3 rounded-xl text-sm font-medium transition-all duration-200 ${
                        sports.includes(activity)
                          ? 'bg-gradient-to-r from-[#F59E0B] to-[#F97316] text-white'
                          : 'bg-white/20 text-white/80 hover:bg-white/30'
                      }`}
                    >
                ...
                <div className="relative rounded-xl overflow-hidden bg-gradient-to-b from-black/50 to-black/80 border border-white/20">
                  ...
                  <div className="h-48 relative overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      ...
                      >
                        <h3 className="text-4xl font-black text-white mb-2">{mascot || 'YourUsername'}</h3>
                        <p className="text-xl text-white/90">{schoolName} {mascot}</p>
                      </motion.div>
                    </div>
                  </div>
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div className="bg-white/10 rounded-lg p-3">
                        ...
                        <p className="text-white font-semibold capitalize">{theme}</p>
                      </div>
                      <div className="bg-white/10 rounded-lg p-3">
                        <p className="text-white/60 text-sm">Activities</p>
                        <p className="text-white font-semibold">{sports.length} Selected</p>
                      </div>
                    </div>
            ...
            <div className="flex justify-between items-center mt-8">
              ...
              {step < 5 ? (
                <button
                  onClick={nextStep}
                  disabled={
                    (step === 1 && (!mascot || !schoolName || !mascot)) ||
                    (step === 3 && sports.length === 0) ||
